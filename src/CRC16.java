...
public class CRC16 implements Checksum {
	private static final int table[] = { 0x0000, 0x1021, 0x2042, 0x3063,
	...
	public long getValue() {
		return this.getCRC() & 0xFFFFL;
	}

	protected void setCRC(int newCRC) {
		this.crc = newCRC;
	}
	...
	public void update(byte[] b, int off, int len) {
		for (int i = off; i < len; i++) {
			// values from b[] aren't ment to be signed
			// the following code corrects this problem:
			if (b[i] < 0)
				this.update((b[i] & 0x7F) + 0x80);
			else
				this.update(b[i]);
	...
	public void update(int b) {
		int old_crc = this.getCRC();
		int new_crc = (old_crc << 8) ^ table[((old_crc >> 8) ^ b) & 0xFF];
		this.setCRC(new_crc);
	}
